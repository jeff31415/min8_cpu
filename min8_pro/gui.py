"""Tkinter-based GUI simulator/debugger for Min8-Pro."""
...
DISASM_LINE_RE = re.compile(r"^([0-9A-F]{4}:)\s+([0-9A-F]{2})\s+([^;]+)(.*)$")
...
REGISTER_GROUPS = (
    ...
    ("Mode", ("EXT16", "R0SEL", "R7SEL"), {"frame": "#44311A", "title": "#FFE8C7", "text": "#FFF4E5"}),
)
...
WORD_FIELDS = {"R0", "R7", "PC", "Pending"}
BYTE_FIELDS = {"R1", "R2", "R3", "R4", "R5", "R6", "IOSEL"}
BIT_FIELDS = {"Z", "C", "EXT16", "R0SEL", "R7SEL"}


class Min8ProGui(tk.Tk):
    def __init__(self, initial_path: str | None = None) -> None:
        super().__init__()
        self.title("Min8-Pro GUI Simulator")
        self.geometry("1540x940")
        self.minsize(1320, 820)

        self.session = Min8ProSession()
        self.running = False
        self.selected_memory_address: int | None = None
        self.memory_page_base = 0
        ...
        self.breakpoint_var = tk.StringVar(value="0x0000")
        self.edit_target_var = tk.StringVar(value="R0")
        self.edit_value_var = tk.StringVar(value="0x0000")
        self.mem_addr_var = tk.StringVar(value="0x0000")
        self.mem_value_var = tk.StringVar(value="0x00")
        self.memory_page_var = tk.StringVar(value="0x0000")

        self.field_value_labels: dict[str, tk.Label] = {}
        self.field_base_colors: dict[str, str] = {}
        self.memory_cells: dict[int, tk.Label] = {}
        self.memory_row_labels: dict[int, tk.Label] = {}
    ...
    def _build_right_panel(self, parent: ttk.Frame) -> None:
        ...
        for group_index, (title, fields, palette) in enumerate(REGISTER_GROUPS):
            ...
            frame.grid(row=group_index // columns, column=group_index % columns, sticky="nsew", padx=4, pady=4)
        for column in range(columns):
            register_root.grid_columnconfigure(column, weight=1)
    ...
    def _build_edit_frame(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(frame, text="Apply State", command=self.on_apply_state_edit).grid(
        ...
        ttk.Button(frame, text="Apply Memory", command=self.on_apply_memory_edit).grid(
        ...
        ttk.Button(frame, text="Clear All", command=self.on_clear_breakpoints).grid(
    ...
    def _build_memory_frame(self, parent: ttk.Frame) -> None:
        ...
        for label, color in (
            ...
            swatch = tk.Label(
                ...
                text=label,
                bg=color,
                fg="#FFFFFF" if label != "Changed" else "#2A2111",
                padx=6,
                pady=2,
            )
        ...
        ttk.Entry(controls, textvariable=self.memory_page_var, width=10).pack(side=tk.LEFT, padx=(4, 6))
        ttk.Button(controls, text="Go", command=self.on_memory_go).pack(side=tk.LEFT, padx=2)
        ttk.Button(controls, text="PC Page", command=self.on_memory_to_pc).pack(side=tk.LEFT, padx=2)
        ttk.Button(controls, text="R7 Page", command=self.on_memory_to_r7).pack(side=tk.LEFT, padx=2)

        grid = tk.Frame(frame, bg="#10161D")
        grid.pack(fill=tk.BOTH, expand=True)

        tk.Label(grid, text="", bg="#10161D", fg="#7E94A7", width=7).grid(row=0, column=0, padx=1, pady=1)
        for column in range(16):
            tk.Label(grid, text=f"{column:X}", bg="#10161D", fg="#7E94A7", width=4).grid(
                row=0, column=column + 1, padx=1, pady=1
            )

        for row in range(16):
            row_label = tk.Label(grid, text="0000", bg="#10161D", fg="#7E94A7", width=7)
            row_label.grid(row=row + 1, column=0, padx=1, pady=1)
            self.memory_row_labels[row] = row_label
            for column in range(16):
                ...
                cell.bind("<Button-1>", lambda _event, rel=offset: self.on_select_memory_address(rel))
                self.memory_cells[offset] = cell
    ...
    def on_load_asm(self) -> None:
        path = filedialog.askopenfilename(
            title="Open Min8-Pro Assembly",
            filetypes=[("Min8-Pro assembly", "*.asm *.s *.min8"), ("All files", "*.*")],
        )
        if path:
    ...
    def on_load_bin(self) -> None:
        path = filedialog.askopenfilename(
            title="Open Min8-Pro Binary Image",
    ...
    def on_reset(self) -> None:
        self.running = False
        self.session.reset()
        self._jump_memory_to_address(self.session.current_address)
    ...
    def on_toggle_breakpoint(self) -> None:
        try:
            address = self._parse_word(self.breakpoint_var.get(), allow_empty=False)
        except ValueError as exc:
            messagebox.showerror("Invalid Breakpoint", str(exc))
            return
        enabled = self.session.toggle_breakpoint(address)
        self._set_status(f"{'Added' if enabled else 'Removed'} breakpoint at 0x{address:04X}")
        self.refresh_all()
    ...
    def on_apply_state_edit(self) -> None:
        target = self.edit_target_var.get().upper()
        try:
            value = self._parse_state_value(target, self.edit_value_var.get())
            self.session.edit_state(target, value)
        except ValueError as exc:
            messagebox.showerror("Invalid State Edit", str(exc))
            return
        self._set_status(f"Updated {target} to {self._format_target_value(target, value)}")
        self.refresh_all()

    def on_apply_memory_edit(self) -> None:
        try:
            address = self._parse_word(self.mem_addr_var.get(), allow_empty=False)
        ...
        self._jump_memory_to_address(address)
        self._set_status(f"Updated memory[0x{address:04X}] = 0x{value:02X}")
        self.refresh_all()
    ...
    def on_memory_go(self) -> None:
        try:
            address = self._parse_word(self.memory_page_var.get(), allow_empty=False)
        except ValueError as exc:
            messagebox.showerror("Invalid Memory Page", str(exc))
            return
        self._jump_memory_to_address(address)
        self._set_status(f"Viewing memory page 0x{self.memory_page_base:04X}")
        self.refresh_all()

    def on_memory_to_pc(self) -> None:
        self._jump_memory_to_address(self.session.current_address)
        self._set_status(f"Viewing PC page 0x{self.memory_page_base:04X}")
        self.refresh_all()

    def on_memory_to_r7(self) -> None:
        self._jump_memory_to_address(self._visible_r7_address())
        self._set_status(f"Viewing R7 page 0x{self.memory_page_base:04X}")
        self.refresh_all()
    ...
    def on_source_double_click(self, event: tk.Event[tk.Misc]) -> None:
        ...
        if address is None:
            return
        self.breakpoint_var.set(f"0x{address:04X}")
    ...
    def on_select_state_target(self, target: str) -> None:
        ...
        if value is not None:
            self.edit_value_var.set(self._format_target_value(target, value))

    def on_select_memory_address(self, offset: int) -> None:
        address = (self.memory_page_base + offset) & 0xFFFF
        self.selected_memory_address = address
        self.mem_addr_var.set(f"0x{address:04X}")
        self.mem_value_var.set(f"0x{self.session.cpu.state.memory[address]:02X}")
        self.breakpoint_var.set(f"0x{address:04X}")
        self.refresh_all()
    ...
    def _load_path(self, path: Path) -> None:
        try:
            ...
            return
        self.running = False
        self.title(f"Min8-Pro GUI Simulator - {path.name}")
        self._jump_memory_to_address(self.session.current_address)
    ...
    def _handle_stop_without_step(self) -> None:
        if self.session.last_stop_reason == "breakpoint" and self.session.last_stop_address is not None:
            self._set_status(f"Breakpoint at 0x{self.session.last_stop_address:04X}")
    ...
    def _refresh_registers(self) -> None:
        ...
        if self.session.cpu.state.pending is not None:
            pending_address = self.session.current_address
        values = {
            **{f"R{i}": self.session.cpu.state.registers[i] for i in range(8)},
            ...
            "EXT16": self.session.cpu.state.ext16,
            "R0SEL": self.session.cpu.state.r0_sel,
            "R7SEL": self.session.cpu.state.r7_sel,
            "Pending": pending_address,
        }
        for field, label in self.field_value_labels.items():
            value = values[field]
            text = "-" if value is None else self._format_field_value(field, value)
            label.configure(text=text)

            base_color = self.field_base_colors[field]
            bg = base_color
            fg = "#EAF6FF"
            if field.startswith("R") and field not in {"R0SEL", "R7SEL"}:
    ...
    def _highlight_disassembly(self, widget: tk.Text, text: str) -> None:
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            widget.tag_add("opcode", f"{line_number}.6", f"{line_number}.8")

            instruction_text = match.group(3)
            mnemonic = instruction_text.strip().split(None, 1)[0] if instruction_text.strip() else ""
            mnemonic_start = line.find(mnemonic, 9)
            if mnemonic:
                widget.tag_add(
                    "mnemonic",
                    f"{line_number}.{mnemonic_start}",
                    f"{line_number}.{mnemonic_start + len(mnemonic)}",
                )
                if mnemonic == ".byte":
    ...
    def _refresh_memory(self) -> None:
        base = self.memory_page_base & 0xFF00
        self.memory_page_var.set(f"0x{base:04X}")
        current = self.session.current_address
        pointer = self._visible_r7_address()
        for row, row_label in self.memory_row_labels.items():
            row_label.configure(text=f"{(base + row * 16) & 0xFFFF:04X}")

        for offset, label in self.memory_cells.items():
            address = (base + offset) & 0xFFFF
    ...
    def _refresh_current_instruction(self) -> None:
        ...
        if pending is not None:
            self.current_instr_var.set(f"Pending @ 0x{current:04X}: {pending.decoded.instruction_text}")
            return
        if self.session.last_result is not None and self.session.last_result.status == "halted":
            self.current_instr_var.set(f"HALT @ 0x{current:04X}")
            return
        ...
        self.current_instr_var.set(f"PC @ 0x{current:04X}: {text}")
    ...
    def _jump_memory_to_address(self, address: int) -> None:
        self.memory_page_base = address & 0xFF00
        self.memory_page_var.set(f"0x{self.memory_page_base:04X}")

    def _selected_channel(self) -> int:
        return int(self.channel_var.get(), 0) & 0xFF

    def _visible_r7_address(self) -> int:
        value = self.session.cpu.state.registers[7]
        return value & (0xFFFF if self.session.cpu.state.ext16 else 0x00FF)
    ...
    def _field_value_for_target(self, target: str) -> int | None:
        ...
        if target == "EXT16":
            return self.session.cpu.state.ext16
        if target == "R0SEL":
            return self.session.cpu.state.r0_sel
        if target == "R7SEL":
            return self.session.cpu.state.r7_sel
        return None

    def _format_field_value(self, field: str, value: int) -> str:
    ...
    def _format_target_value(self, target: str, value: int) -> str:
        field = target.upper()
        if field in WORD_FIELDS:
    ...
    def _parse_state_value(self, target: str, text: str) -> int:
        if target in WORD_FIELDS:
            return self._parse_word(text, allow_empty=False)
        if target in BYTE_FIELDS:
            return self._parse_byte(text, allow_empty=False)
        if target in BIT_FIELDS:
            token = text.strip()
            if token not in {"0", "1"}:
                raise ValueError(f"{text!r} must be 0 or 1")
            return int(token)
        raise ValueError(f"unknown target {target!r}")

    def _parse_word(self, text: str, *, allow_empty: bool) -> int:
        token = text.strip()
        if not token:
            if allow_empty:
                return 0
        ...
        if not 0 <= value <= 0xFFFF:
            raise ValueError(f"{text!r} is out of 16-bit range")
        return value

    def _parse_byte(self, text: str, *, allow_empty: bool) -> int:
        value = self._parse_word(text, allow_empty=allow_empty)
        if value > 0xFF:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Launch the Min8-Pro GUI simulator")
    parser.add_argument("program", nargs="?", help="optional .asm or .bin file to load at startup")
    args = parser.parse_args(argv)
    app = Min8ProGui(initial_path=args.program)
    app.mainloop()
    return 0
