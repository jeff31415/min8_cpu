"""Assembler for the Min8-Pro ISA."""
...
LONG_JUMP_PSEUDOS = {"LJMP": "JMP", "LJZ": "JZ", "LJC": "JC", "LJNZ": "JNZ"}
IMAGE_SIZE = 65536


class AssemblerError(Exception):
    """Raised when Min8-Pro assembly cannot be parsed or encoded."""
...
def assemble_source(source: str, *, source_name: str = "<string>") -> AssemblyResult:
    ...
    for parsed in parsed_lines:
        ...
        if emitted:
            ...
            if location > IMAGE_SIZE:
                raise AssemblerError(parsed.line_number, "program exceeds 64K address space")
...
def format_listing(result: AssemblyResult) -> str:
    ...
    for entry in result.listing:
        if entry.address is None:
            lines.append(f"        {'':<23} {entry.source_text}")
            continue
        bytes_text = " ".join(f"{value:02X}" for value in entry.bytes_out)
        lines.append(f"{entry.address:04X}:  {bytes_text:<23} {entry.source_text}")
    return "\n".join(lines) + ("\n" if lines else "")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Assemble Min8-Pro source code")
...
def _conservative_instruction_size(parsed: ParsedLine) -> int:
    ...
    if parsed.name == "LI16":
        _require_arg_count(parsed, 2)
        return 7
    if parsed.name in LONG_JUMP_PSEUDOS:
        _require_arg_count(parsed, 2)
        return 8
...
def _instruction_size(parsed: ParsedLine, symbols: dict[str, int]) -> int:
    ...
    if parsed.name == "LI16":
        _require_arg_count(parsed, 2)
        _eval_word(parsed.args[1], symbols, parsed.line_number)
        return 7
    if parsed.name in LONG_JUMP_PSEUDOS:
        _require_arg_count(parsed, 2)
        _eval_word(parsed.args[1], symbols, parsed.line_number)
...
def _encode_instruction(parsed: ParsedLine, symbols: dict[str, int]) -> list[int]:
    ...
    if name in SELECTOR_OPCODE_BY_MNEMONIC:
        _require_arg_count(parsed, 0)
        return [0xC0 | SELECTOR_OPCODE_BY_MNEMONIC[name]]
    ...
    if name == "LI16":
        _require_arg_count(parsed, 2)
        register = _parse_register(parsed.args[0], parsed.line_number)
        value = _eval_word(parsed.args[1], symbols, parsed.line_number)
        return _encode_li16(register, value, parsed.line_number)

    if name in LONG_JUMP_PSEUDOS:
        _require_arg_count(parsed, 2)
        scratch = _parse_register(parsed.args[0], parsed.line_number)
        target = _eval_word(parsed.args[1], symbols, parsed.line_number)
        return _encode_long_jump(name, scratch, target, parsed.line_number)
...
def _encode_li16(register: int, value: int, line_number: int) -> list[int]:
    if register not in {0, 7}:
        raise AssemblerError(line_number, "LI16 only supports R0 or R7")
    value &= 0xFFFF
    low_byte = value & 0xFF
    high_byte = (value >> 8) & 0xFF
    target = "R0" if register == 0 else "R7"
    select_low = 0xC0 | SELECTOR_OPCODE_BY_MNEMONIC[f"{target}L"]
    select_high = 0xC0 | SELECTOR_OPCODE_BY_MNEMONIC[f"{target}H"]
    ldi_l = LDI_OPCODE_BASE[f"LDI_L_{target}"]
    ldi_h = LDI_OPCODE_BASE[f"LDI_H_{target}"]
    return [
        select_low,
        ldi_l | (low_byte & 0x0F),
        ldi_h | ((low_byte >> 4) & 0x0F),
        select_high,
        ldi_l | (high_byte & 0x0F),
        ldi_h | ((high_byte >> 4) & 0x0F),
        select_low,
    ]


def _encode_long_jump(name: str, scratch: int, target: int, line_number: int) -> list[int]:
    if scratch not in {0, 7}:
        raise AssemblerError(line_number, f"{name} scratch register must be R0 or R7")
    branch_name = LONG_JUMP_PSEUDOS[name]
    return _encode_li16(scratch, target, line_number) + [MEM_CTRL_OPCODE_BASE[branch_name] | scratch]
...
def _emit_bytes(
    *,
    ...
    if address < 0 or address >= IMAGE_SIZE:
        raise AssemblerError(line_number, f"address 0x{address & 0xFFFF:04X} is out of range")
    if address + len(values) > IMAGE_SIZE:
        raise AssemblerError(line_number, "program exceeds 64K address space")
    for offset, value in enumerate(values):
        target = address + offset
        if occupied[target]:
            raise AssemblerError(line_number, f"address 0x{target:04X} is written more than once")
...
def _advance_location(location: int, amount: int, line_number: int) -> int:
    ...
    if not 0 <= next_location <= IMAGE_SIZE:
        raise AssemblerError(line_number, "program exceeds 64K address space")
...
def _define_symbol(symbols: dict[str, int], name: str, value: int, line_number: int, *, wrap: bool = True) -> None:
    ...
    symbols[name] = value & 0xFFFF if wrap else value


def _eval_address(expr: str, symbols: dict[str, int], line_number: int) -> int:
    value = _eval_expr(expr, symbols, line_number)
    if not 0 <= value <= 0xFFFF:
        raise AssemblerError(line_number, f"value {value} does not fit in 16 bits")
    return value


def _eval_word(expr: str, symbols: dict[str, int], line_number: int) -> int:
