"""Debugger/session helpers for Min8-Pro."""
...
class Min8ProSession:
    ...
    def load_source(self, source_text: str, *, source_name: str = "<string>") -> AssemblyResult:
        ...
        self.loaded_program = self._build_loaded_program(
            ...
            source_path=None,
            disassembly_addresses=assembly.used_addresses,
        )
    ...
    def load_image_file(self, path: str | Path) -> None:
        ...
        if len(image) > IMAGE_SIZE:
            raise ValueError("Binary image is larger than 64K")
        padded = image + bytes(IMAGE_SIZE - len(image))
        self.loaded_program = self._build_loaded_program(
            image=padded,
            ...
            disassembly_addresses=range(len(image)),
        )
        self.breakpoints.clear()
        self.reset()
    ...
    def edit_state(self, target: str, value: int) -> None:
        ...
        if normalized in REGISTER_INDEX:
            index = REGISTER_INDEX[normalized]
            masked = value & (0xFFFF if index in {0, 7} else 0xFF)
            self.cpu.state.registers[index] = masked
            self.last_register_changes.add(index)
            return

        if normalized == "PC":
            self.cpu.state.pc = value & 0xFFFF
            self.last_special_changes.add("PC")
            return

        if normalized == "IOSEL":
            self.cpu.state.iosel = value & 0xFF
            self.last_special_changes.add("IOSEL")
            return

        if normalized in {"Z", "C", "EXT16", "R0SEL", "R7SEL"}:
            field_map = {
                "Z": "z",
                "C": "c",
                "EXT16": "ext16",
            ...
            self.cpu.state.__dict__[field_map[normalized]] = int(bool(value))
    ...
    def edit_memory(self, address: int, value: int) -> None:
        ...
        self.cpu.state.memory[masked_address] = value & 0xFF
        self.last_memory_changes.add(masked_address)

    @property
    def current_address(self) -> int:
        pending = self.cpu.state.pending
        if pending is not None:
            return pending.pc_before & (0xFFFF if self.cpu.state.ext16 else 0x00FF)
        if self.last_result is not None and self.last_result.status == "halted":
            mask = 0xFFFF if self.last_result.ext16_before else 0x00FF
            return self.last_result.pc_before & mask
        if self.cpu.state.ext16:
            return self.cpu.state.pc & 0xFFFF
        return self.cpu.state.pc & 0x00FF
    ...
    def disassembly_text(self) -> str:
        ...
        return format_disassembly(disassemble_image(self.cpu.state.memory, addresses=range(IMAGE_SIZE)))

    def source_line_for_address(self, address: int) -> int | None:
        if self.loaded_program is None:
            return None
        return self.loaded_program.address_to_source_line.get(address & 0xFFFF)
    ...
    def disassembly_line_for_address(self, address: int) -> int | None:
        if self.loaded_program is None:
            return None
        return self.loaded_program.address_to_disassembly_line.get(address & 0xFFFF)
    ...
    def memory_dump(self, *, base: int = 0, rows: int = 16) -> str:
        lines: list[str] = []
        start = base & 0xFFFF
        for row in range(rows):
            row_base = (start + row * 16) & 0xFFFF
            chunk = [self.cpu.state.memory[(row_base + offset) & 0xFFFF] for offset in range(16)]
            lines.append(f"{row_base:04X}: " + " ".join(f"{value:02X}" for value in chunk))
        return "\n".join(lines) + "\n"
    ...
    def _build_loaded_program(
        ...
        *,
        ...
        disassembly_addresses: range | tuple[int, ...],
    ) -> LoadedProgram:
        if assembly is not None and assembly.used_addresses:
            ...
            addresses = assembly.used_addresses
        else:
            source_line_to_address = {}
            address_to_source_line = {}
            addresses = disassembly_addresses

        disassembly_lines = disassemble_image(image, addresses=addresses, symbols=assembly.symbols if assembly else None)
    ...
    def _build_source_maps(self, listing: tuple[ListingLine, ...]) -> tuple[dict[int, int], dict[int, int]]:
        ...
        for entry in listing:
            ...
            for offset in range(len(entry.bytes_out)):
                address_to_source_line[(entry.address + offset) & 0xFFFF] = entry.line_number
    ...
    def _record_step_changes(self, result: StepResult) -> None:
        ...
        if result.iosel_before != result.iosel_after:
            self.last_special_changes.add("IOSEL")
        if result.ext16_before != result.ext16_after:
            self.last_special_changes.add("EXT16")
        if result.r0sel_before != result.r0sel_after:
            self.last_special_changes.add("R0SEL")
        if result.r7sel_before != result.r7sel_after:
            self.last_special_changes.add("R7SEL")
        if result.status == "blocked":
            self.last_special_changes.add("Pending")

        sequential_next = (
            (result.pc_before + 1) & 0xFFFF if result.ext16_before else (result.pc_before + 1) & 0x00FF
        )
