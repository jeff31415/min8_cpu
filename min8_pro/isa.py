"""ISA constants and decode helpers for Min8-Pro."""
...
SELECTOR_OPCODE_BY_MNEMONIC = {mnemonic: opcode for opcode, mnemonic in SELECTOR_MNEMONICS.items()}
...
def decode_opcode(opcode: int, *, pc: int | None = None) -> DecodedInstruction:
    ...
    top3 = opcode >> 5
    if top3 == 0b110:
        subopcode = opcode & 0x1F
        mnemonic = ALU_MNEMONICS.get(subopcode)
        if mnemonic is not None:
            return DecodedInstruction(
                opcode=opcode,
                family="alu",
                mnemonic=mnemonic,
                alu_subopcode=subopcode,
            )
        mnemonic = SELECTOR_MNEMONICS.get(subopcode)
        if mnemonic is not None:
            return DecodedInstruction(
                opcode=opcode,
                family="selector",
        ...
        raise IllegalInstruction(
            opcode=opcode,
            pc=pc or 0,
            message="Reserved ALU/selector opcode",
        )
