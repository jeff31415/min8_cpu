"""Reference Min8 instruction interpreter."""
...
class Min8CPU:
    """Reference instruction-level simulator for the Min8 ISA."""
    ...
    def reset(self, *, clear_memory: bool = False) -> None:
        ...
        if clear_memory:
            self.state.memory[:] = bytes(256)

    def load_image(self, image: bytes | bytearray | list[int], *, start: int = 0) -> None:
        data = bytes(image)
        if len(data) > 256:
            raise ValueError("Image is larger than Min8 address space")
        base = start & 0xFF
        for offset, value in enumerate(data):
            self.state.memory[(base + offset) & 0xFF] = value
    ...
    def step(self) -> StepResult:
        ...
        if self.state.pending is None:
            ...
            opcode = self.state.memory[pc_before]
            self.state.pc = (self.state.pc + 1) & 0xFF
    ...
    def _execute_pending(self, pending: PendingInstruction) -> StepResult:
        ...
        if instruction.family == "mov":
            ...
            self._write_register(instruction.dest, self.state.registers[instruction.src], register_writes)
            status = "retired"
            ...
            if instruction.mnemonic == "ST":
                self._store(self.state.registers[7], self.state.registers[reg], memory_writes)
            elif instruction.mnemonic == "LD":
                self._write_register(reg, self.state.memory[self.state.registers[7]], register_writes)
            elif instruction.mnemonic == "JMP":
                self.state.pc = self.state.registers[reg]
                ...
                addr = self.state.registers[7]
                self._store(addr, self.state.registers[reg], memory_writes)
                self._write_register(7, (addr + 1) & 0xFF, register_writes)
            elif instruction.mnemonic == "LD+":
                if reg == 7:
                    raise IllegalInstruction(
                        opcode=pending.opcode,
                        pc=pending.pc_before,
                        message="LD+ R7 encoding is reserved for HALT",
                    )
                addr = self.state.registers[7]
                self._write_register(reg, self.state.memory[addr], register_writes)
                self._write_register(7, (addr + 1) & 0xFF, register_writes)
            else:
            ...
            current = self.state.registers[instruction.ldi_target]
            if instruction.ldi_high:
                value = ((instruction.imm4 & 0x0F) << 4) | (current & 0x0F)
            else:
                value = instruction.imm4 & 0x0F
            self._write_register(instruction.ldi_target, value, register_writes)
            status = "retired"

        elif instruction.family == "alu":
            value, carry = self._execute_alu(instruction)
            self._write_register(0, value, register_writes)
            ...
            try:
                if instruction.mnemonic == "SETIO":
                    self.state.iosel = self.state.registers[reg]
                elif instruction.mnemonic == "GETIO":
                    self._write_register(reg, self.state.iosel, register_writes)
                elif instruction.mnemonic == "IN":
                    value = self.io.read(channel)
                    self._write_register(reg, value, register_writes)
                    io_transfer = IOTransfer("in", channel, value)
    ...
    def _write_register(self, index: int, value: int, writes: list[RegisterWrite]) -> None:
        ...
        writes.append(RegisterWrite(index=index, before=before, after=value))

    def _store(self, address: int, value: int, writes: list[MemoryWrite]) -> None:
        address &= 0xFF
        value &= 0xFF
        before = self.state.memory[address]
        self.state.memory[address] = value
        writes.append(MemoryWrite(address=address, before=before, after=value))
