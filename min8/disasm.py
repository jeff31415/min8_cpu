"""Disassembler helpers for the Min8 ISA."""
    ...
    *,
    ...
    if symbols:
        for name, address in symbols.items():
            address_to_symbol.setdefault(address & 0xFF, []).append(name)

    lines: list[DisassemblyLine] = []
    seen: set[int] = set()
    for raw_address in target_addresses:
        address = raw_address & 0xFF
...
def format_disassembly(lines: tuple[DisassemblyLine, ...] | list[DisassemblyLine]) -> str:
    ...
    for line in lines:
        ...
        rows.append(f"{line.address:02X}: {line.opcode:02X}  {line.text}{suffix}")
    return "\n".join(rows) + ("\n" if rows else "")
